...
const Orders = () => {
  ...
  const updateItem = (idx, key, val) => {
    ...
    if (key === "batchId") {
      ...
      if (b) newItems[idx].price = b.sellingPrice;
    }
  ...
  return (
    <Box className="orders-page">
      ...
      <Box className="page-header">
        ...
        {(role === "USER" || isManager) && (
          <ButtonBase onClick={() => setCreate({ open: true, data: { ...emptyOrder, items: [{ ...emptyItem }] } })} disableRipple
      ...
      <Box className="stat-cards-row">
        {Object.entries(STATUS_META).map(([key, meta]) => (
          <Box className="stat-card" key={key}>
            <Box className="stat-card-icon" sx={{ background: key === "PENDING" ? "#fef9c3" : key === "APPROVED" ? "#dcfce7" : key === "REJECTED" ? "#fee2e2" : "#e0f2fe" }}>
              <meta.Icon sx={{ color: key === "PENDING" ? "#ca8a04" : key === "APPROVED" ? "#16a34a" : key === "REJECTED" ? "#ef4444" : "#0284c7", fontSize: 22 }} />
            </Box>
            <Box>
          ...
          <Table size="small">
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={6} align="center" sx={{ py: 6, color: "#94a3b8", fontFamily: "Poppins, sans-serif" }}>No orders found</TableCell></TableRow>
              ) : (
                filtered.map((o) => {
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        {(isAdmin || isManager) && o.status === "PENDING" && (
                          <Box sx={{ display: "flex", gap: 0.75 }}>
                            <Tooltip title="Approve"><ButtonBase className="action-btn edit" onClick={() => updateStatus(o.id, "APPROVED")} disableRipple><ThumbUpRounded sx={{ fontSize: 14 }} /></ButtonBase></Tooltip>
                            <Tooltip title="Reject"><ButtonBase className="action-btn delete" onClick={() => updateStatus(o.id, "REJECTED")} disableRipple><ThumbDownRounded sx={{ fontSize: 14 }} /></ButtonBase></Tooltip>
                          </Box>
                        )}
                      </TableCell>
                    </TableRow>
            ...
            {create.data.items.map((item, idx) => (
              <Box key={idx} sx={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1.2fr auto", gap: 1.5, alignItems: "center", p: 2, border: "1px solid #e2e8f0", borderRadius: 3 }}>
                ...
                <Box>
                  ...
                  <TextField fullWidth size="small" type="number" value={item.quantity} onChange={(e) => updateItem(idx, "quantity", e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
                </Box>
                <Box>
                  <Typography className="dialog-field-label">Price</Typography>
                  <TextField fullWidth size="small" type="number" value={item.price} onChange={(e) => updateItem(idx, "price", e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
                </Box>
                <IconButton onClick={() => removeItem(idx)} color="error" sx={{ mt: 2 }}><DeleteRounded /></IconButton>
              </Box>
            ))}
          </Box>
          <ButtonBase onClick={addItem} sx={{ mt: 2, color: "#7d2ae8", fontWeight: 600, fontSize: "0.875rem", fontFamily: "Poppins, sans-serif" }}>+ Add Another Item</ButtonBase>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <ButtonBase onClick={() => setCreate({ open: false, data: emptyOrder })} sx={{ px: 2.5, py: 1, borderRadius: "50px", border: "1.5px solid #e2e8f0", color: "#64748b", fontWeight: 600 }}>Cancel</ButtonBase>
          <ButtonBase onClick={handleCreate} sx={{ px: 2.5, py: 1, borderRadius: "50px", background: "linear-gradient(135deg,#7d2ae8,#a855f7)", color: "#fff", fontWeight: 600, boxShadow: "0 4px 12px rgba(125,42,232,0.35)" }}>Create Order</ButtonBase>
        </DialogActions>
      </Dialog>

      {/* Order Detail Dialog (Existing) */}
      <Dialog open={!!detail} onClose={() => setDetail(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 4, overflow: "hidden" } }}>
        {detail && (
          <>
                ...
                return (
                  <Box key={step} sx={{ display: "flex", alignItems: "flex-start", flex: 1 }}>
                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
                      ...
                      <Typography className={`timeline-label${done ? " done" : ""}`}>{STATUS_META[step]?.label}</Typography>
                    </Box>
                    {i < TIMELINE.length - 1 && <Box className={`timeline-line${done && timelineIdx(detail.status) > i ? " done" : ""}`} sx={{ mt: "14px", flex: 1 }} />}
                  </Box>
                );
              })}
            </Box>
            <DialogContent>
              <Table size="small">
                <TableHead><TableRow>{["Product", "Batch", "Qty", "Price"].map((h) => (<TableCell key={h} sx={{ fontWeight: 700, color: "#7d2ae8", fontSize: "0.75rem" }}>{h}</TableCell>))}</TableRow></TableHead>
                <TableBody>
                  {(detail.items || []).map((item, idx) => (
                    <TableRow key={idx}>
                      <TableCell>{item.product?.name || item.productId}</TableCell>
                      <TableCell color="textSecondary">{item.batch?.batchNo || item.batchId}</TableCell>
            ...
            <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
              <ButtonBase onClick={() => setDetail(null)} disableRipple sx={{ px: 2.5, py: 1, borderRadius: "50px", border: "1.5px solid #e2e8f0", color: "#64748b", fontWeight: 600 }}>Close</ButtonBase>
              {(isAdmin || isManager) && detail.status === "PENDING" && (
                <>
                  <ButtonBase onClick={() => updateStatus(detail.id, "REJECTED")} sx={{ px: 2.5, py: 1, borderRadius: "50px", background: "#fee2e2", color: "#ef4444", fontWeight: 600 }}>Reject</ButtonBase>
                  <ButtonBase onClick={() => updateStatus(detail.id, "APPROVED")} sx={{ px: 2.5, py: 1, borderRadius: "50px", background: "linear-gradient(135deg,#7d2ae8,#a855f7)", color: "#fff", fontWeight: 600 }}>Approve</ButtonBase>
                </>
              )}
            </DialogActions>
          </>
